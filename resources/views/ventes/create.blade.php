...
@section('content')
<h2><u>Création d'une vente</u></h2>
<form class="formMessages" action="{{ route('ventes.store') }}" method="POST">
    @csrf
    <div>
    ...
    <div>
        ...
        <input type="text" name="prixHT" id="prixHT" value="{{ old('prixHT') }}">
    ...
    <div>
        ...
        <input name="titre" id="titre" value="{{ old('titre') }}"><br>
        @if ($errors->has('titre'))
            <p>{{ $errors->first('titre')}}</p>
        @endif
    </div>
